...
public class CompanyPracticumSessionUpdateService extends AbstractService<Company, PracticumSession> {
	...
	public void validate(final PracticumSession object) {
		...
		final Date startDate = super.getRequest().getData("initialPeriod", Date.class);
		final Date endDate = super.getRequest().getData("finalPeriod", Date.class);
		...
		super.state(validStart, "initialPeriod", "company.practicum-session.validation.startDate.error.AtLeastOneWeekAntiquity");

		final boolean validEnd = endDate.getTime() >= availableEnd.getTime();
		super.state(validEnd, "finalPeriod", "company.practicum-session.validation.endDate.error.AtLeastOneWeekDuration");
